...
from model.surrogate import surrogate_s1, surrogate_s3
...
class TrainArgs:
    ...
    val_path: str = field(default="/root/autodl-tmp/colornet/freeform_test.mat")
...
def main(
    ...
    logger.info(f"Training script for surrogate", on_rank0=True)
    if rank != -1:
        set_seed(0 + rank)
    ...
    train_set = SurrogateFreeFormDataset(path=train_args.data_path)
    val_set = SurrogateFreeFormDataset(path=train_args.val_path)
    trainer = Trainer(
        model,
        train_set,
        val_set,
