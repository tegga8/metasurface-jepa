"""LeJEPA objective smoke tests (Milestone B final screening, Batch 4 directive):
...
class _LeJEPAStubModel(nn.Module):
    ...
    into the student is observable."""
...
def test_lejepa_stop_grad_target_false(lejepa_model, tiny_batch):
    ...
    the target (student z_y through proj) stays attached to the graph, so
    backward flows into the student encoder — the SIGReg design contract."""
    G, S, M = tiny_batch
    obj = LeJEPAObjective()
    result = obj(lejepa_model, G, S, M)
    mask = result["out"]["mask"]

    # Reference L_J with stop_grad_target=False on the same inputs.
    z_y = lejepa_model.geometry_encoder(G)
    L_ref, _ = jepa_loss(lejepa_model.z_hat, z_y, mask, proj=lejepa_model.proj,
                         stop_grad_target=False)
    assert torch.allclose(result["components"]["L_J"], L_ref, atol=1e-6), (
        "L_J inside LeJEPA must equal a stop_grad_target=False reference")
...
def test_lejepa_loss_composition(lejepa_model, tiny_batch):
    ...
    assert torch.allclose(c["sigreg_ratio"],
                          ...
                          atol=1e-4)
    assert c["sigreg_info"]["test"].startswith("sliced ECF"), c["sigreg_info"]
...
def test_lejepa_sigreg_metadata_complete(lejepa_model, tiny_batch):
    ...
    obj = LeJEPAObjective(lambda_sigreg=0.1, num_slices=8, num_points=16, seed=3)
    result = obj(lejepa_model, *tiny_batch)
    info = result["components"]["sigreg_info"]
    assert isinstance(info, dict), "sigreg_info must stay a dict in components"
    for k in ("num_slices", "num_points", "t_grid", "seed", "mean_phi_dev"):
        assert k in info, f"sigreg_info missing {k}"
